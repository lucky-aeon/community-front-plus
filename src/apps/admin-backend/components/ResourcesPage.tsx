...
import { AdminResourceService } from '@shared/services/api/admin-resource.service';
import { ResourceDTO, ResourceQueryRequest, PageResponse } from '@shared/types';
...
export const ResourcesPage: React.FC = () => {
  ...
  const filteredList = useMemo(() => {
    ...
    return list.filter(r =>
      ...
      r.userId?.toLowerCase().includes(k)
    );
  }, [list, keyword]);
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res: PageResponse<ResourceDTO> = await AdminResourceService.getResources(query);
      setList(res.records);
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3 min-w-0">
            <Input placeholder="搜索文件名/格式/用户ID" value={keyword} onChange={(e) => setKeyword(e.target.value)} />
            <Select
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="min-w-[160px]">用户ID</TableHead>
